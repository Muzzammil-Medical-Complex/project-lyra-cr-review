...
async def test_services_container(test_db, mock_groq_client, mock_chutes_client, 
    ...
    # Set static instances for the services that use them
    DatabaseManager.set_instance(test_db)
    GroqClient.set_instance(mock_groq_client)
    ChutesClient.set_instance(mock_chutes_client)
    LettaService.set_instance(mock_letta_service)
    MemoryManager.set_instance(mock_memory_manager)
    PersonalityEngine.set_instance(mock_personality_engine)
