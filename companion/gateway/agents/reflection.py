...
class ReflectionReport:
    ...
    def add_batch_result(self, batch_num: int, total_users: int, successful_users: int, duration: float):
        ...
        # Check for duplicate batch_num (replace for idempotency)
        existing_index = None
        for i, batch in enumerate(self.batch_results):
            if batch['batch_num'] == batch_num:
                existing_index = i
                break

        # Validate batch_num sequencing
        if self.batch_results:
            max_batch_num = max(batch['batch_num'] for batch in self.batch_results)
            next_expected = max_batch_num + 1

            # Reject out-of-order entries (batch_num less than max, not a duplicate)
            if batch_num < max_batch_num and existing_index is None:
                logging.error(f"Out-of-order batch entry rejected: batch_num {batch_num} < current max {max_batch_num}")
                return

            # Warn about sequence gaps but allow them
            if batch_num > next_expected and existing_index is None:
                logging.warning(f"Batch sequence gap: expected {next_expected}, got {batch_num}")
        ...
        if existing_index is not None:
            self.batch_results[existing_index] = batch_entry
            logging.info(f"Replaced duplicate batch result for batch_num {batch_num}")
        else:
            self.batch_results.append(batch_entry)

        # Maintain sorted order by batch_num
        self.batch_results.sort(key=lambda x: x['batch_num'])
...
class ReflectionAgent:
    ...
    async def identify_memory_themes(self, user_id: str, memories: List[Memory]) -> List[MemoryTheme]:
        ...
        def sanitize_content(content: str) -> str:
            # Remove/escape content that could manipulate the AI
            # Truncate excessively long content and escape quotes
        ...
        sanitized_memories = [sanitize_content(mem.content) for mem in memories]
        ...
        Memories (indexed):
        {chr(10).join([f'{i}: {sanitized_memories[i]}' for i in range(len(memories))])}
    ...
    async def _consolidate_theme_to_semantic(self, user_id: str, theme: MemoryTheme) -> Memory:
        ...
        avg_importance = total_importance / len(theme._related_memories) if theme._related_memories else 0.0
    ...
    def _calculate_emotional_volatility(self, emotion_history: List[PADState]) -> float:
        ...
        total_std = pleasure_std + arousal_std + dominance_std
        return total_std / 3 if pleasure_values or arousal_values or dominance_values else 0.0
