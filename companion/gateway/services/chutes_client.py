...
class ChutesClient:
    ...
    def __init__(
        self,
        api_key: str,
        primary_model: str = "qwen3-80b-a3b-instruct",
        fallback_model: str = "qwen2-72b-instruct",
        ...
        Args:
            ...
            timeout: Request timeout in seconds (default: 30s connect, 60s read)
        """
        ...
        if timeout is not None:
            # Use provided timeout for both connect and read
    ...
    async def _make_api_request(
        ...
        try:
            ...
            self.logger.error(f"Chutes API request error: {e}", exc_info=True)
            raise ServiceUnavailableError(
                service_name="Chutes",
            ...
            self.logger.error(f"Chutes API HTTP error: {e}", exc_info=True)
            raise ServiceUnavailableError(
                service_name="Chutes",
            ...
            self.logger.error(f"Chutes API response parsing error: {e}", exc_info=True)
            raise ServiceUnavailableError(
                service_name="Chutes",
                message=f"Invalid API response: {str(e)}"
            )
        except ServiceUnavailableError:
            # Re-raise our own exception
            raise
        # Programming errors (AttributeError, TypeError, etc.) are intentionally not caught
        # They will propagate to help identify bugs during development
    ...
    async def get_available_models(self) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # The /models endpoint returns a top-level JSON array
                if isinstance(models, list):
                    return [model['id'] for model in models if 'id' in model]
                else:
                    self.logger.warning(f"Unexpected response format from models endpoint: {type(models)}")
