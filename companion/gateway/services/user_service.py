...
class UserService:
    ...
    async def delete_user(self, user_id: str) -> bool:
        ...
        """
        try:
            # First, delete the associated Letta agent
            user_profile = await self.get_user_profile(user_id)
            if user_profile and user_profile.letta_agent_id:
                await self.letta_service.delete_agent(user_profile.letta_agent_id)
    ...
    async def cleanup_inactive_users(self) -> int:
        ...
        """
        try:
            return await self.db.cleanup_inactive_users()
        except Exception as e:
