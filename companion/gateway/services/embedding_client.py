...
class EmbeddingClient:
    ...
    async def embed_text(self, text: str) -> List[float]:
        ...
        """
        try:
            # Create a cache key based on the text and dimensions
            cache_key = f"embedding:{hash(text)}:{self.dimensions}"
            ...
            self.logger.error(f"Unexpected error in embedding service call: {e}")
            raise ServiceUnavailableError(
                service_name="Embedding Service",
    ...
    async def embed_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            self.logger.error(f"Unexpected error in batch embedding service call: {e}")
            raise ServiceUnavailableError(
                service_name="Embedding Service",
                message=f"Batch unexpected error: {str(e)}"
            )
