...
class ImportanceScorer:
    ...
    def __init__(self, groq_client: GroqClient, redis_client=None):
        """
        Initialize the importance scorer with a Groq client and Redis client.
    ...
    async def score_importance(self, content: str, context: Optional[Dict[str, Any]] = None) -> float:
        ...
        """
        try:
            # Create a deterministic cache key using hashlib
            content_hash = hashlib.sha256(content.encode('utf-8')).hexdigest()[:16]
            context_hash = hashlib.sha256(str(context or {}).encode('utf-8')).hexdigest()[:16]
            cache_key = f"importance:{content_hash}:{context_hash}"
            ...
            response = await self.groq_client.chat_completion(
                ...
                max_tokens=50,  # Allow room for numeric score plus brief reasoning
