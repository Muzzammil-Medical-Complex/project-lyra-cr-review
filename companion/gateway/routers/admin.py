...
router = APIRouter(tags=["admin"], dependencies=[Depends(verify_admin)])
...
async def run_system_cleanup(
    ...
    try:
        ...
        # Open a database transaction for atomic cleanup operations
        async with db.get_transaction() as tx:
            if cleanup_type in ['memories', 'all']:
                # Clean up old memories
                cleaned_memories = await memory_manager.cleanup_old_memories(tx=tx)
                cleanup_results['memories_cleaned'] = cleaned_memories
            
            if cleanup_type in ['users', 'all']:
                # Clean up inactive users
                cleaned_users = await user_service.cleanup_inactive_users(tx=tx)
            ...
            # Transaction automatically commits when exiting the context manager
...
@router.post("/maintenance/migrate_memories", response_model=Dict[str, Any])
async def migrate_user_memories(
    ...
    try:
        ...
        if source_user_id == target_user_id:
            raise HTTPException(status_code=400, detail="Source and target users cannot be the same")
