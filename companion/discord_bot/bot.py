...
class CompanionBot(commands.Bot):
    def __init__(self, gateway_url: str = "http://gateway:8000", redis_url: str = "redis://redis:6379"):
        ...
        self.user_locks: Dict[str, asyncio.Lock] = {}  # Prevent concurrent processing per user
    ...
    async def on_message(self, message):
        ...
        if isinstance(message.channel, discord.DMChannel) or message.guild is not None:
            ...
            if lock.locked():
                ...
                # Poll for lock to clear (up to 30 seconds)
                timeout_seconds = 30
    ...
    async def process_proactive_messages(self):
        ...
        try:
            ...
            if lock and lock.locked():
                # If user is currently being processed, requeue the message with retry counter
                if retries < 3:  # Max 3 retries
                    msg_info["retries"] = retries + 1
                    await self.proactive_queue.put(msg_info)
                else:
                    self.logger.warning(f"Dropping proactive message for user {user_id} after {retries} retries")
                return
            ...
            async with self.session.get(f"{self.gateway_url}/admin/users?user_id={user_id}") as response:
                if response.status == 200:
                    ...
                    try:
                        ...
                        if user:
                            new_profile_data["discord_username"] = user.name
...
async def run_discord_bot(token: str, gateway_url: str = "http://gateway:8000", redis_url: str = "redis://redis:6379"):
    """
    ...
    bot = CompanionBot(gateway_url=gateway_url, redis_url=redis_url)
...
if __name__ == "__main__":
    ...
    # Run the bot
    asyncio.run(run_discord_bot(DISCORD_TOKEN, GATEWAY_URL, REDIS_URL))