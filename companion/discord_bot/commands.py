...
class CompanionCommands(commands.Cog):
    def __init__(self, bot, gateway_url: str = "http://gateway:8000"):
        ...
        self.command_usage = {}  # Track command usage for rate limiting
    ...
    async def companion_chat(self, ctx: discord.ApplicationContext, message: str):
        ...
        # Check rate limit
        allowed, rate_limit_msg = await check_rate_limit(user_id, "companion_chat")
        if not allowed:
    ...
    async def view_personality(self, ctx: discord.ApplicationContext):
        ...
        # Check rate limit
        allowed, rate_limit_msg = await check_rate_limit(user_id, "personality")
        if not allowed:
            await ctx.respond(rate_limit_msg, ephemeral=True)
            return
        ...
        try:
            ...
            if personality_data:
                ...
                current_pad = personality_data.get("current_pad", {})
    ...
    async def view_memories(self, ctx: discord.ApplicationContext, query: Optional[str] = None, limit: Optional[int] = 5):
        ...
        # Check rate limit
        allowed, rate_limit_msg = await check_rate_limit(user_id, "memories")
        if not allowed:
            await ctx.respond(rate_limit_msg, ephemeral=True)
            return
    ...
    async def view_needs(self, ctx: discord.ApplicationContext):
        ...
        # Check rate limit
        allowed, rate_limit_msg = await check_rate_limit(user_id, "needs")
        if not allowed:
            await ctx.respond(rate_limit_msg, ephemeral=True)
            return
    ...
    async def view_stats(self, ctx: discord.ApplicationContext):
        ...
        # Check rate limit
        allowed, rate_limit_msg = await check_rate_limit(user_id, "stats")
        if not allowed:
            await ctx.respond(rate_limit_msg, ephemeral=True)
            return
        ...
        try:
            ...
            # Call gateway API to get user profile (which includes stats)
            user_profile = await self._call_gateway_api(
                f"{self.gateway_url}/admin/users?user_id={user_id}"
            )
