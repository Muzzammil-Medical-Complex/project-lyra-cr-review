...
Provides endpoints for generating text embeddings with caching.
"""
import os
import asyncio
import logging
...
# Redis configuration (client initialized in startup event)
...
async def startup_event():
    ...
    try:
        ...
        raise RuntimeError(f"Redis initialization failed with unexpected error: {e}") from e
...
async def embed_text(request: EmbedRequest, _: str = Depends(verify_api_key)):
    """
...
async def embed_batch(request: EmbedBatchRequest, _: str = Depends(verify_api_key)):
    """
